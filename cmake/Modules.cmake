...
set(NATIVE_SOURCE_DIR "${PROJECT_SOURCE_DIR}/Engine/cpp")
...
if (BUILD_BENCHMARKS)
    set(NANOBENCH_INCLUDE_DIR "${NATIVE_THIRD_PARTY_DIR}/nanobench")
endif()
...
function(add_modules_library)
    ...
    file(GLOB CPP_BENCHMARKS CONFIGURE_DEPENDS "${LIB_PATH}/*.bench.cpp")
    file(GLOB CPP_SRC_FILES CONFIGURE_DEPENDS "${LIB_PATH}/*.cpp")
    if (CPP_UNIT_TESTS)
        list(REMOVE_ITEM CPP_SRC_FILES ${CPP_UNIT_TESTS})
    endif()
    if (CPP_BENCHMARKS)
        list(REMOVE_ITEM CPP_SRC_FILES ${CPP_BENCHMARKS})
    endif()
    ...
    if(BUILD_BENCHMARKS)
        foreach(BENCH_FILE IN LISTS CPP_BENCHMARKS)
            string(REPLACE "${LIB_DIR}/" "" BENCH_TARGET "${BENCH_FILE}")
            string(REPLACE ".bench.cpp" "_bench" BENCH_TARGET "${BENCH_TARGET}")
            string(MAKE_C_IDENTIFIER "${BENCH_TARGET}" BENCH_TARGET)
            if (NOT BENCH_TARGET MATCHES ".*${LIB_TARGET}.*")
                string(PREPEND BENCH_TARGET "${LIB_TARGET}_")
            endif()
            add_executable(${BENCH_TARGET} EXCLUDE_FROM_ALL ${BENCH_FILE})
            set_target_properties(${BENCH_TARGET} PROPERTIES CXX_SCAN_FOR_MODULES ON)
            target_compile_features(${BENCH_TARGET} PRIVATE cxx_std_23)
            target_include_directories(${BENCH_TARGET} PRIVATE ${NANOBENCH_INCLUDE_DIR})
            target_link_libraries(${BENCH_TARGET} PRIVATE ${LIB_TARGET})
            set_property(GLOBAL APPEND PROPERTY DRACO_BENCH_TARGETS ${BENCH_TARGET})
            message(STATUS "Benchmark ${BENCH_TARGET}")
        endforeach()
    endif()
